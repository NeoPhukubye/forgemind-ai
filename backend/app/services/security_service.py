...
class SecurityOrchestrator:
    async def analyze_and_patch(self, vulnerable_code: str) -> dict:
        ...
        try:
            architecture_report = llm.complete(architect_prompt, vulnerable_code)
        except LLMServiceError as exc:
            return {"error": str(exc)}
        ...
        try:
            patched_code = llm.complete(generator_prompt, generator_input)
        except LLMServiceError as exc:
        ...
        try:
            review_verdict = llm.complete(reviewer_prompt, reviewer_input)
        except LLMServiceError as exc:
            return {
                "error": str(exc),
